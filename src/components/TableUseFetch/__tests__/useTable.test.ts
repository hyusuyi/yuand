...
describe("useTable (TableUseFetch) Hook 测试", () => {
  it("应该返回 table 实例", () => {
  ...
  it("clear 方法应该清空数据", () => {
    ...
    expect(table.useStore.getState().data).toEqual({
      list: [1, 2, 3],
      total: 3,
    });
  ...
  it("refresh 方法应该设置 ready 为 true", () => {
    const { result } = renderHook(() => useTable());
    const [table] = result.current;

    expect(table.useStore.getState().ready).toBe(false);

    act(() => {
      table.refresh();
    });

    expect(table.useStore.getState().ready).toBe(true);
  });
  ...
  it("run 方法应该存在", () => {
    ...
    expect(table.run).toBeDefined();
    expect(typeof table.run).toBe("function");
  });
